...
export class CVPage extends BasePage {
  ...
  async init() {
    ...
    if (headerSection) {
      ...
      const headerContent = this.createHeaderSection(communityData, cvData.about, cvData.skills);
      if (headerContent) {
    ...
    if (certificatesSection && cvData.certificates && cvData.certificates.length > 0) {
      ...
      const certificatesList = this.createCertificatesSection(cvData.certificates);
      if (certificatesList) {
